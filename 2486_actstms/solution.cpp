 ...
 * Если при сдвиге указателей i == t.length(), то ранее мы бы вернули true, т.к является подстрокой, но это буквально
 * означает, что нам не надо добавлять никаких символов, чтобы t стала подстрокой => вернем 0
 * Ранее если цикл кончался, то мы бы вернули false, тогда это означало, что t не подстрока, в нашем случае
